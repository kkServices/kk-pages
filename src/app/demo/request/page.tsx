...
const MockRequest: React.FC<MockRequestProps> = async (_props: MockRequestProps) => {
  const [error, result] = await to<any, BusinessException>(getExampleProfile('19953', '2'))
  if (error) {
    const { response } = error
    return <BusinessError message={response.message} code={response.code} requestId={response.requestId} />
  }
  return (
