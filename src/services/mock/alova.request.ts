...
export class WarpAlova<S, E, RC, RE, RH> {
  ...
  private processConfig<R, T = unknown>(config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>): AlovaMethodCreateConfig<R, T, RC, RH> {
    const meta: AlovaMeta = {
  ...
  Get<R, T = unknown>(url: string, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
    return this.instance.Get<R, T>(url, this.processConfig(config))
  }

  Post<R, T = unknown>(url: string, data?: RequestBody, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
    return this.instance.Post<R, T>(url, data, this.processConfig(config))
  }

  Patch<R, T = unknown>(url: string, data?: RequestBody, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
    return this.instance.Patch<R, T>(url, data, this.processConfig(config))
  }

  Put<R, T = unknown>(url: string, data?: RequestBody, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
    return this.instance.Put<R, T>(url, data, this.processConfig(config))
  }

  Delete<R, T = unknown>(url: string, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
    return this.instance.Delete<R, T>(url, this.processConfig(config))
  }

  Options<R, T = unknown>(url: string, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
    return this.instance.Options<R, T>(url, this.processConfig(config))
  }

  Head<R, T = unknown>(url: string, config?: ConfigWithMeta<Partial<AlovaMeta>, R, T, RC, RH>) {
